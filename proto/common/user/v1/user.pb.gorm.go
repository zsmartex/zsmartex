...
type UserORM struct {
	PasswordDigest string      `gorm:"type:character varying;not null" json:"password_digest,omitempty"`
	Username       null.String `gorm:"unique;index:idx_users_on_username" json:"username,omitempty"`
	State          string      `gorm:"type:character varying;default:pending;not null" json:"state,omitempty"`
	Level          int32       `gorm:"type:integer;default:0;not null;index:index_users_on_level" json:"level,omitempty"`
	Data           *UserData   `gorm:"type:jsonb" json:"data,omitempty"`
	UpdatedAt      *time.Time  `gorm:"type:timestamp;not null" json:"updated_at,omitempty"`
	Id             *int64      `gorm:"type:bigint;primaryKey;autoIncrement" json:"id,omitempty"`
	Uid            string      `gorm:"type:character varying;unique;index:index_users_on_uid" json:"uid,omitempty"`
	Otp            bool        `gorm:"type:boolean;default:false;not null" json:"otp,omitempty"`
	Role           string      `gorm:"type:character varying;default:member;not null" json:"role,omitempty"`
	CreatedAt      *time.Time  `gorm:"type:timestamp;not null" json:"created_at,omitempty"`
}
...
func (m *User) ToORM(ctx context.Context) (UserORM, error) {
	...
	to.Uid = m.Uid
	to.PasswordDigest = m.PasswordDigest
...
type UserCredentialsORM struct {
	UpdatedAt      *time.Time `gorm:"type:timestamp;not null" json:"updated_at,omitempty"`
	UserId         *int64     `gorm:"type:bigint;not null;uniqueIndex:idx_users_on_user_id_and_type" json:"user_id,omitempty"`
	ValueIndex     int64      `gorm:"type:bigint;not null;uniqueIndex:index_user_credentials_on_value_index" json:"value_index,omitempty"`
	ValueEncrypted string     `gorm:"type:character varying;not null" json:"value_encrypted,omitempty"`
	Type           string     `gorm:"type:character varying(8);not null;uniqueIndex:idx_users_on_user_id_and_type" json:"type,omitempty"`
	CreatedAt      *time.Time `gorm:"type:timestamp;not null" json:"created_at,omitempty"`
}

// TableName overrides the default tablename generated by GORM
func (UserCredentialsORM) TableName() string {
	return "user_credentials"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *UserCredentials) ToORM(ctx context.Context) (UserCredentialsORM, error) {
	to := UserCredentialsORM{}
	var err error
	if prehook, ok := interface{}(m).(UserCredentialsWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	if m.UserId != nil {
		if v, err := resource.DecodeInt64(nil, m.UserId); err != nil {
			return to, err
		} else {
			to.UserId = &v
		}
	}
	to.ValueIndex = m.ValueIndex
	to.ValueEncrypted = m.ValueEncrypted
	to.Type = UserCredentials_Type_name[int32(m.Type)]
	if m.CreatedAt != nil {
		t := m.CreatedAt.AsTime()
		to.CreatedAt = &t
	}
	...
	if posthook, ok := interface{}(m).(UserCredentialsWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *UserCredentialsORM) ToPB(ctx context.Context) (UserCredentials, error) {
	to := UserCredentials{}
	var err error
	if prehook, ok := interface{}(m).(UserCredentialsWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	to.ValueIndex = m.ValueIndex
	to.ValueEncrypted = m.ValueEncrypted
	to.Type = UserCredentials_Type(UserCredentials_Type_value[m.Type])
	if m.CreatedAt != nil {
		to.CreatedAt = timestamppb.New(*m.CreatedAt)
	}
	if m.UpdatedAt != nil {
	...
	if posthook, ok := interface{}(m).(UserCredentialsWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type UserCredentials the arg will be the target, the caller the one being converted from

// UserCredentialsBeforeToORM called before default ToORM code
type UserCredentialsWithBeforeToORM interface {
	BeforeToORM(context.Context, *UserCredentialsORM) error
}

// UserCredentialsAfterToORM called after default ToORM code
type UserCredentialsWithAfterToORM interface {
	AfterToORM(context.Context, *UserCredentialsORM) error
}

// UserCredentialsBeforeToPB called before default ToPB code
type UserCredentialsWithBeforeToPB interface {
	BeforeToPB(context.Context, *UserCredentials) error
}

// UserCredentialsAfterToPB called after default ToPB code
type UserCredentialsWithAfterToPB interface {
	AfterToPB(context.Context, *UserCredentials) error
}
...
func DefaultApplyFieldMaskUser(ctx context.Context, patchee *User, patcher *User, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*User, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"PasswordDigest" {
			patchee.PasswordDigest = patcher.PasswordDigest
			continue
		}
...
// DefaultCreateUserCredentials executes a basic gorm create call
func DefaultCreateUserCredentials(ctx context.Context, in *UserCredentials, db *gorm.DB) (*UserCredentials, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(UserCredentialsORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(UserCredentialsORMWithAfterCreate_); ok {
		if err = hook.AfterCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
type UserCredentialsORMWithBeforeCreate_ interface {
	BeforeCreate_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type UserCredentialsORMWithAfterCreate_ interface {
	AfterCreate_(context.Context, *gorm.DB) error
}

// DefaultApplyFieldMaskUserCredentials patches an pbObject with patcher according to a field mask.
func DefaultApplyFieldMaskUserCredentials(ctx context.Context, patchee *UserCredentials, patcher *UserCredentials, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*UserCredentials, error) {
	if patcher == nil {
		return nil, nil
	} else if patchee == nil {
		return nil, errors.NilArgumentError
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"ValueIndex" {
			patchee.ValueIndex = patcher.ValueIndex
			continue
		}
		if f == prefix+"ValueEncrypted" {
			patchee.ValueEncrypted = patcher.ValueEncrypted
			continue
		}
		if f == prefix+"Type" {
...
// DefaultListUserCredentials executes a gorm list call
func DefaultListUserCredentials(ctx context.Context, db *gorm.DB) ([]*UserCredentials, error) {
	in := UserCredentials{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(UserCredentialsORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	db, err = gorm1.ApplyCollectionOperators(ctx, db, &UserCredentialsORM{}, &UserCredentials{}, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(UserCredentialsORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db); err != nil {
			return nil, err
		}
	}
	db = db.Where(&ormObj)
	ormResponse := []UserCredentialsORM{}
	if err := db.Find(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(UserCredentialsORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse); err != nil {
			return nil, err
		}
	}
	pbResponse := []*UserCredentials{}
	for _, responseEntry := range ormResponse {
		temp, err := responseEntry.ToPB(ctx)
		if err != nil {
			return nil, err
...
type UserCredentialsORMWithBeforeListApplyQuery interface {
	BeforeListApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type UserCredentialsORMWithBeforeListFind interface {
	BeforeListFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type UserCredentialsORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm.DB, *[]UserCredentialsORM) error
}
